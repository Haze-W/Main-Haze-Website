...
export function AuthFormPanel({ mode }: AuthFormPanelProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignup) {
        const result = await signup(email, name || undefined, password);
        if (result.error) {
  ...
  const handleGoogle = async () => {
    setLoading(true);
    try {
      const result = await signInWithGoogle(isSignup ? "/onboarding" : "/dashboard");
  ...
  const handleGithub = async () => {
    setLoading(true);
    try {
      const result = await signInWithGithub(isSignup ? "/onboarding" : "/dashboard");
      if (result.error) toast.show(result.error.message, "error");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
        <h1 className={styles.formTitle}>
          {isSignup ? "Create your account" : "Sign in to Haze"}
        </h1>

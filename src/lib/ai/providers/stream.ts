...
/**
 * Ollama /api/chat with stream:true — newline-delimited JSON; each chunk may include
 * `message.content` as an incremental assistant delta.
 */
export function ollamaNDJSONToNormalizedStream(upstreamBody: ReadableStream<Uint8Array> | null): ReadableStream<Uint8Array> {
  if (!upstreamBody) {
    return new ReadableStream({
      start(controller) {
        const enc = new TextEncoder();
        ...
        while (true) {
          ...
          for (const rawLine of parts) {
            ...
            try {
              ...
              if (json.error) {
                controller.enqueue(enc.encode(`data: ${JSON.stringify({ error: json.error })}\n\n`));
                continue;
              }
              const c = json.message?.content;
              if (typeof c === "string" && c) {
                controller.enqueue(
                  enc.encode(`data: ${JSON.stringify({ content: c, reasoning: "" })}\n\n`)
                );
              }
            } catch {
